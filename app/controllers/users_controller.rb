class UsersController < ApplicationController
  ...
  def requests
  	...
  	@submitted_requests = @current_user.submitted_requests
  	...
  	#@active_requests = Request.where('owner_user_id = ? AND status = ?', @current_user.id, "Awaiting Response")
  	#@inactive_requests = Request.where('requester_user_id = ? AND status = ?', @current_user.id, "Awaiting Response")
  	#@loaned = Request.where('(requester_user_id = ? OR owner_user_id = ?) AND status = ?', @current_user.id, @current_user.id, "Accepted")
  	#@completed = Request.where('(requester_user_id = ? OR owner_user_id = ?) AND (status != ? AND status != ?)', @current_user.id, @current_user.id, "Awaiting Response", "Accepted")
  end
end